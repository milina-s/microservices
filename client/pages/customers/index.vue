    ...
    <div class="w-full sm:px-6">
      ...
      >
        <div class="sm:flex items-center justify-between">
          ...
          >
            <span>Customers</span>
            <span v-if="customers.length > 0" class="text-sm text-gray-600 font-medium">Total: {{ customers.length }}</span>
          </p>
          <div>
            <button
            ...
            >
              <nuxt-link to="/customers/create" class="text-sm font-medium leading-none text-white">
                New customer
              </nuxt-link>
            </button>
          </div>
        ...
        <table class="w-full whitespace-nowrap">
          <thead>
            ...
            >
              ...
              <th class="font-normal text-left pl-8">Phone Number</th>
              <th class="font-normal text-left pl-12">Address</th>
              <th class="font-normal text-left pl-4">City</th>
            </tr>
          </thead>
          <tbody class="w-full">
            <tr
              v-for="(order, idx) in customers"
              :id="idx"
              @click="navigateTo(`/customers/${order.id}`)"
              tabindex="0"
            ...
            >
              ...
              <td class="pl-8 cursor-pointer text-gray-800 font-semibold">
                {{ order.phone }}
              </td>
              ...
              <td>
                <button v-on:click.stop @click="() => deleteCustomer(order.id)" class="text-red-500 p-2 border-transparent border bg-gray-100 hover:bg-gray-200 cursor-pointer rounded focus:outline-none focus:border-gray-800 focus:shadow-outline-gray" aria-label="delete table" role="button">
...
const fetchCustomers = async () => {
  const res = await axios.get(`${config.public.api}api/customer`);
  customers.value = res.data;
};

const deleteCustomer = async id => {
  const res = await axios.delete(`${config.public.api}api/customer/${id}`);
  if (res.status === 200) {
