      ...
      <div class="relative">
        <div class="absolute">
          <div class="">
            ...
            <p class="my-2 text-gray-800 dark:text-indigo-600">Sorry about that! Go back to the orders page to view all orders.</p>
            <button @click="navigateTo('/orders')" class="sm:w-full lg:w-auto my-2  rounded md py-4 px-8 text-center bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-700 focus:ring-opacity-50">Take me there!</button>
          </div>
        </div>
        <div>
    ...
    <div class="container mx-auto bg-white mt-10 rounded px-4">
      <div class="xl:w-full border-b border-gray-300 py-5">
        <div class="flex w-11/12 mx-auto xl:w-full xl:mx-0 items-center">
          <p class="text-lg text-gray-800 font-bold">Order #{{ route.params.id }}</p>
...
onMounted(async () => {
  const { data } = await axios.get(`${config.public.api}api/orders/get/${route.params.id}`);
  if (!data) doesntExist.value = true;
  customerId.value = data.clientId;
  itemIds.value = new Set(data.itemId);
});
...
const saveItem = async () => {
  ...
  const res = await axios.put(`${config.public.api}api/orders/update/${route.params.id}`, data);
  if (res.status === 200) {
    navigateTo('/orders');
  }
};
